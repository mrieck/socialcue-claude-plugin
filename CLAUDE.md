...
- **Subscription + auto-refresh** (`lib/license-refresh.js`): Pro is a $19/mo
  ...
  `license connect <token>` (or pastes it into the dashboard License panel,
  ...
  (`ensureBridge`, fire-and-forget) and when the key nears expiry. **This and
  the opt-in Postiz integration (see Content Strategy below) are the only
  outbound calls the plugin makes; the license refresh carries only the token** —
  never brands, pages, or opportunities. It's
...
- Free vs Pro line: discovery, the queue UI, and all of Content Strategy
  ...
  free; **assisted posting** and **reply performance tracking** are Pro. Never
  gate discovery.
...
- **Postiz is FREE on purpose (2026-08-11, user decision): Social Cue plans to
  ...
  Pro remains: assisted posting + reply performance tracking.
