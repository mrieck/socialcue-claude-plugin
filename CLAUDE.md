...
results stream into the dashboard live during a run. `bridge open` opens the
dashboard in the **dedicated Social Cue Chrome profile** (new tab over CDP
`PUT /json/new` if it's up, else launches it on the dashboard URL) — the OS
default browser is only a fallback when Chrome isn't installed. Rules that
must not drift:
...
- Assisted-posting pre-fill covers HN + old/new Reddit + X + Indie Hackers +
  Product Hunt + Threads (`bridge/post-intent.js` `prefillReply`); unknown platforms
...
- Surface: the dashboard **Analytics** tab's "Submitted replies" list (Pro) —
  metric chips on reply rows plus a Performance section in the detail pane ←
  `GET /api/performance` (`bridge/routes.js`, 403 `pro_required` on free, same
  gate as posting; the dashboard swallows the 403 so free users see the list
  undecorated). The tab was called "Submitted" until 2026-09-10.

## Founder invitations (Pro "invite" pillar, 2026-09-12)

The tracking plan targets **five individually matched invitations a week**: a
solo founder whose post is an *explicit ask* for what a brand does gets a
public reply first, then a personal DM. Discovery now does the legwork.

- **Brief**: on Pro, `brief` appends a `FOUNDER INVITATIONS` section
  (`buildInvitationsSection` in shared `prompts.js`: each brand's
  `idealCustomer` / `requirements` / `disqualifiers` — new `BrandSchema`
  fields, editable in Settings and drafted by `/socialcue-setup` — the
  per-run cap `config.maxLeadsPerRun` (0-10, default 3, 0 = off, Settings
  slider) and the people investigated in the last 90 days). The static rubric
  and DM rules live in `scoring-core.md` ("Founder invitations (Pro)"). No
  orchestrator step: `lead brief` prints the section alone for inspection.
- **Subagent** (`agents/discovery-subagent.md`): collect the reply, then
  investigate the author in a second tab (≤ 4 turns: bio, recent posts, one
  product-site visit — it now has the tab tools), then `collect_lead` (MCP,
  `mcp/browser-server/server.js`; Pro-gated server-side, 90-day person dedupe
  via `db.isPersonSeen`, links the reply by `findOpportunityByUrl`). Never
  follow/like/message during discovery.
- **Store**: `leads` table (`lib/db.js`; `addLead`, `queryLeads` joins the
  linked reply's status + latest `reply_checks.reply_count`, `updateLead`
  stamps `invited_at`/`responded_at`/`setup_at`/`first_result_at` on the FIRST
  arrival at that status, `recentLeadProfiles`, `countInvitedInWeek`,
  `leadCounts` → `counts.invitations` badge). Person dedupe is its own column
  (`normalized_profile_url`), never `seen_urls` — those feed the brief's
  skip-list. Statuses: `candidate → replied → invited → responded →
  setup_done → first_result → closed`, plus `skipped`.
- **Bridge** (`bridge/lead-routes.js`): `GET /api/leads` (free tier answers
  `{leads: [], pro: false}` like destinations), `PATCH /api/leads/:id`
  (status/userDm/objection/note), `POST /api/leads/:id/open` (opens the
  profile in the dedicated Chrome via `openInDedicatedBrowser`, extracted from
  `post-intent.js`). **Nothing sends a DM**: the dashboard copies the draft to
  the clipboard and the user pastes it into the platform's composer.
- **Dashboard**: no Invitations tab (removed 2026-09-14 — it crowded the
  sidebar for a UI shaped exactly like a conversation). `Invitations.tsx`
  renders open leads as a block **pinned to the bottom of the Conversations
  queue** (`.invites-pinned`, sticky; brand/platform filters apply; sorted by
  fit) and `LeadDetail` in the same right pane (one selection across rows and
  people — `selectOpportunity`/`selectLead` in `App.tsx`). Row ✕ = **Dismiss**
  → status `skipped`; skipped and closed leads never show again. The sidebar's
  Conversations badge adds `counts.invitations`. Detail: evidence/ask/DM
  editor/pipeline buttons/log dates; "Message" stays disabled until the linked
  reply is `posted` (reply first, DM second) and re-checks after a queue status
  change. Goals: rule type `invitation` counts `invited_at` in the week.
- CLI: `lead brief|list|show|status|update`. Tests: `tests/leads.test.mjs`,
  `tests/lead-brief.test.mjs`.

## Weekly goals (Analytics tab, free)

The dashboard's **Analytics** tab (renamed from "Submitted" on 2026-09-10) opens
with a **weekly goals board** over the posted-replies list. It exists for the
product direction "help a solo founder run a measurable weekly marketing
routine": a goal is a per-week target ("3 TikTok videos") with an optional
**auto-rule** that counts rows already in the store, plus manual **"Log one"**
entries for things Social Cue never sees (SEO articles, a demo video). Auto and
logged counts are shown separately. Weeks run **from 00:00 on
`config.weekStartsOn` (0 = Sunday default, 1 = Monday; Settings → Discovery
runs) for seven days, in the bridge process's local time** (`weekBounds` /
`configuredWeekStart` in `lib/goals.js`); the board header states the exact
rollover instant (`week.endsAt`, `daysLeft`) because "the week just started"
was ambiguous (2026-09-14). **Auto items are listed, not just counted**
(`matchRule` → `autoItems`), and any one of them can be excluded per goal
(`goal_exclusions`, `db.setGoalExclusion`, `PATCH /api/goals/:id/items/:kind/:itemId
{excluded}`, CLI `goals items|exclude|include`) — the count and the sidebar
badge both honour exclusions. The `count*InWeek` db helpers are `list*InWeek(…).length`.

- **Store** (`lib/db.js`): `goals` (title, target, `brand_id` nullable — the
  auto-rule's brand filter, `rule` JSON or NULL = manual-only, `sort_order`,
  `archived_at`) and `goal_entries` (`at`, `note`, `source` manual|cli|<agent>,
  `ref` = optional idempotency key with a unique `(goal_id, ref)` index so a
  producer can re-log the same item safely). No FK cascade (`foreign_keys` is
  never enabled) — `deleteGoal` removes entries in a transaction.
- **Rules** (closed set, validated by `validateRule`): `content` (Content
  Library rows in `scheduled|published`, time = `COALESCE(published_at,
  scheduled_for, sent_at)`; filters `platform` = Postiz `__type` in
  `variants`/`settings` (`instagram` also matches `instagram-standalone`),
  `source`, `titlePrefix`, `excludeTitlePrefix`), `post` (`product_posts` in
  `submitted|live`, time = `COALESCE(submitted_at, verified_at, updated_at)`;
  `destinationHost` LIKE, `postType`), `reply` (`opportunities` kind=reply
  status=posted, time = `COALESCE(posted_at, posted_intent_at, created_at)`;
  `platform` normalised through `normalizePlatform` so "Twitter/X", "twitter"
  and "x" all count). Scheduled items count in the week they are scheduled
  for (a planning board, by design). Memes are only distinguishable by the
  `meme-` title prefix — keep that convention in the crosspost/memelab flow.
- **`opportunities.posted_at`** (added 2026-09-10) is stamped once by
  `setOpportunityStatus(id, 'posted')` — every path (dashboard PATCH,
  extension push, CLI, auto-submit) funnels through it. Legacy rows are NULL.
- **Bridge**: `bridge/goals-routes.js` (`GET/POST /api/goals`, `POST
  /api/goals/seed`, `PATCH/DELETE /api/goals/:id`, `POST/DELETE
  …/:id/entries[/:entryId]`), all free, every write bumps the change counter.
  `/api/changes` `counts.analytics` = active goals still short this week (the
  sidebar badge); `counts.submitted` is kept for stale builds.
- **CLI**: `goals list [--week YYYY-MM-DD --json] | add <json> | log <id|title
  substring> [--note --at --ref --source] | rm | seed`. `goals log` is the hook
  for producer skills (e.g. seoblog: `goals log "trysocialcue" --ref
  seoblog:<site>:<slug>`). The starter set (`STARTER_GOALS`) is the founder's
  own cadence and is only added explicitly (button or `goals seed`), never on
  first open.
- **Dashboard**: `AnalyticsPanel.tsx` = `GoalsBoard.tsx` + the old Submitted
  pieces (FilterBar + QueueTable + DetailPane), board inside the scrolling
  `.queue-wrap` so the detail pane keeps its column. Tests:
  `tests/goals.test.mjs`.
...
`open_link({url})` (`mcp/browser-server/open-link.js`) is an **optional**
URL-addressed link activator kept for clients whose approval layer needs the
destination in the call (Codex, 2026-09-14). It is deliberately NOT on the
`click(ref)` / `act` path: ref clicks stay trusted mouse clicks resolved by
`backendNodeId`, and no prompt steers Claude agents toward `open_link` (it
refuses buttons, `target=_blank`, aria-hidden ancestors — too strict for feeds).
`npm run test:browser` covers it in isolated headless Chrome.
