<!--
  SINGLE SOURCE OF TRUTH for Social Cue's scoring "brain".
  Vendored into the plugin Skill (skills/scoring-brain) and used by the
  extension's scoring pass. Edit here, then run `npm run sync`. Do not edit the
  vendored copies directly — they are overwritten.

  This file is the STATIC guidance only. The per-run brief (brands, platforms,
  seen URLs, strategy weights) is assembled at runtime by
  buildAutonomousDiscoveryPrompt() in prompts.js.
-->
...
**Collect opportunities that clear the run's minimum score** (default 6 average; the run brief states the current bar — the user can lower it from Settings when runs come back thin). Score first, decide second: report the real average, never round up to clear the bar, and when a thread lands right on it say in the context what makes it borderline. The user sorts by score, so an honest 5 beats an inflated 7.
...
- **Threads is casual and reply-driven.** The For You feed is interest-ranked, so
  small accounts reach real audiences and replies/quote-posts travel much further
  than likes. Drafts must be short (1-2 sentences), conversational, meme-literate —
  anything that reads like LinkedIn dies. Never put a link in a reply (Threads
  wraps and downranks them); name the product in plain text only when the thread
  is asking for tools.
...
When the run brief carries a **FOUNDER INVITATIONS** section, some threads are also
candidates for a personal invitation: the author is asking, in their own words, for the
thing a brand does. The brief names the cap per run and each brand's ideal customer.
Outside that section, never investigate people.

- **What an explicit ask looks like.** "Launched two weeks ago, zero users, what am I
  doing wrong", "where do you promote a dev tool", "how did you get your first 10
  customers", "any tool that finds Reddit threads to reply to". Complaints about the
  category, hot takes, and "building in public" updates are not asks — reply to those,
  don't investigate them.
- **Fit rubric (score honestly, 1–10).** Released product with a URL; the person does
  their own distribution (solo or a tiny team, not an agency or a marketer-for-hire);
  the brand's stated requirements are met or plausible; no disqualifier. Missing product
  → 3 at most. Everything present and the ask is exactly the brand's problem → 8+.
- **Investigate, don't stalk.** Bio, a scroll of recent posts, one product-site visit.
  Four turns, in a second tab. Evidence is short public facts you saw, quoted or
  paraphrased — never contact details, never guesses about the person.
- **Public reply first.** The invitation follows the posted reply; on Reddit, DMs from a
  fresh account get flagged, so note "ask permission publicly first" as a disqualifier
  unless the sub or the person invites DMs.
- **DM draft rules.** ≤ 80 words. Open by quoting their post. Name their product.
  State the requirement in the first two sentences (e.g. "it runs on a Claude Code
  subscription, so only worth it if you already have one"). One product only. One
  yes/no question. Offer setup help and ask for blunt feedback on whether the results
  were actually useful. Promise no outcomes, cite no numbers you can't back, and never
