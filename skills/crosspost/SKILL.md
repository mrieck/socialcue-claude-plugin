...
}' | node "${CLAUDE_PLUGIN_ROOT}/lib/cli.js" content add -
```

Then (`<id>` is what `content add` printed):
...
## 7. First run on a new account

On the **first** crosspost for a TikTok channel, do a `--type draft` pass and
have the user check the draft in TikTok: unaudited API apps get privacy forced
to `SELF_ONLY` regardless of what was sent. Verify the draft shows the intended
privacy before the first real `--type schedule`.
