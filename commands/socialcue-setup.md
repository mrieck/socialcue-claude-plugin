...
- **idealCustomer** — one or two sentences: who this is really for, as a person
  (e.g. "solo founder with a released SaaS who does their own marketing"). Used by
  the Pro founder-invitations investigator to judge whether someone is worth a DM.
- **requirements** — what someone must already have to use it (e.g. "a Claude Code
  subscription", "macOS"). Stated up front in every invitation so people can judge
  fit before investing setup time.
- **disqualifiers** — who to never reach out to (e.g. "agencies, students, anyone
  without a released product").

Present the draft compactly and ask what to adjust (they always know something the
site doesn't — especially voice and who the customer really is). If they have no
website, interview for the same fields instead, and leave url empty.
...
node "${CLAUDE_PLUGIN_ROOT}/lib/cli.js" brand add --name '<name>' --url '<url>' \
  ...
  --ideal-customer '<idealCustomer>' --requirements '<requirements>' --disqualifiers '<disqualifiers>'
```
...
**Hacker News**; **Threads** — interest-ranked feed, great reach for new
accounts; **Founder communities** — Indie Hackers + Product Hunt, great
...
`hackernews`, `threads`, `indiehackers`, `producthunt` ("Founder communities" = both) — or a
