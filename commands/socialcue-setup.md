...
use it later to find reusable images/copy when submitting the brand to directories.
...
### 5b. Directory signups (Pro only — skip on free unless they ask)

Run `node "${CLAUDE_PLUGIN_ROOT}/lib/cli.js" license show`. If Pro is active (or
the user asks about getting listed on directories), set up the signup email:

- Ask which email they want their brand's directory accounts on — one address
  for everything (Product Hunt, BetaList, …). Explain in a sentence: the agent
...
- Save it: `node "${CLAUDE_PLUGIN_ROOT}/lib/cli.js" dir email <address>` — it
