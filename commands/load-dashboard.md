...
2. **Open the dashboard.** Run:
   ...
   already running, otherwise it launches it; falls back to the default browser
   only if Chrome isn't installed). Relay
...
4. **Weekly goals.** Run:
   `node "${CLAUDE_PLUGIN_ROOT}/lib/cli.js" goals list`
   If goals exist, relay this week's progress in a line (how many goals are met,
   which are furthest behind). If it prints "no goals yet", mention that the
   Analytics tab can set weekly publishing targets (or `goals seed` for a
   starter set) — don't seed on the user's behalf.
