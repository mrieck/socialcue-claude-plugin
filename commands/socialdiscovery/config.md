...
- **Directory signup email (Pro):** `node "${CLAUDE_PLUGIN_ROOT}/lib/cli.js" dir email <address> [--webmail url]`
  — the address the user is signed into in the Social Cue Chrome; `dir email` alone shows it.

## Editing other settings

For platforms, strategy weights, the browser CDP URL, `autoLike`, `engagementRatio`,
`utmTagging` (auto-append utm_* params to own-site links when scheduling through
Postiz; default true — also a switch in the dashboard's Settings tab), etc., read `config.json` (use `config path` to find it) and edit the JSON directly
