...
test('brand listing facts: add/update flags round-trip into the subject brief; repeated --link accumulates', async () => {
  const store = makeStore();
  cli(store, ['config', 'init']);
  cli(store, ['brand', 'add', '--name', 'Acme', '--url', 'https://acme.dev', '--pricing', 'Free; Pro $19/mo']);
  cli(store, ['brand', 'update', 'Acme', '--pricing-url', 'https://acme.dev/pricing', '--repo-url', 'https://github.com/x/acme',
    '--launched', '2026-08-01', '--founders', 'Ada, Bob', '--link', 'linkedin=https://linkedin.com/company/acme', '--link', 'x=https://x.com/acme']);
  const config = JSON.parse(readFileSync(path.join(store, 'config.json'), 'utf8'));
  const brand = config.brands[0];
  assert.equal(brand.pricingSummary, 'Free; Pro $19/mo');           // add kept it
  assert.equal(brand.pricingUrl, 'https://acme.dev/pricing');
  assert.equal(brand.launchedOn, '2026-08-01');
  assert.deepEqual(brand.socialLinks, { linkedin: 'https://linkedin.com/company/acme', x: 'https://x.com/acme' });
  // A later partial update must not blank fields it did not mention.
  cli(store, ['brand', 'update', 'Acme', '--tagline', 'new tag']);
  const again = JSON.parse(readFileSync(path.join(store, 'config.json'), 'utf8')).brands[0];
  assert.equal(again.tagline, 'new tag');
  assert.equal(again.founders, 'Ada, Bob');
  assert.throws(() => cli(store, ['brand', 'update', 'Acme', '--link', 'not-a-url']), /--link must look like/);
  assert.throws(() => cli(store, ['brand', 'update', 'Nope', '--tagline', 'x']), /No brand matches/);

  process.env.SOCIALCUE_DIR = store;
  const { resolveSubject } = await import('../lib/post-subject.js');
  const b = resolveSubject({ subject: 'brand:Acme' }, config);
  assert.equal(b.subject.repoUrl, 'https://github.com/x/acme');
  assert.equal(b.subject.socialLinks.x, 'https://x.com/acme');
});

test('the CLI refuses to create a store outside the project (no config, no SOCIALCUE_DIR)', () => {
  const scratch = mkdtempSync(path.join(tmpdir(), 'socialcue-noconfig-'));
  const env = { ...process.env };
  delete env.SOCIALCUE_DIR;
  const run = (args) => execFileSync('node', [CLI, ...args], { cwd: scratch, env, encoding: 'utf8', stdio: ['pipe', 'pipe', 'pipe'] });
  assert.throws(() => run(['content', 'list']), /No Social Cue store found/);
  assert.ok(!existsSync(path.join(scratch, '.socialdiscovery')), 'must not create a stray store');
  // `config show` and `config init` are still allowed there.
  assert.match(run(['config', 'show']), /not created yet/);
});

test('venue identity: multi-tenant hosts keep their path so GitHub lists do not collide', async () => {
  const { venueIdentityForUrl } = await import('../lib/venues-sync.js');
  assert.deepEqual(venueIdentityForUrl('https://www.saashub.com/submit'), { host: 'saashub.com', id: 'saashub-com', name: 'saashub.com', multiTenant: false });
  const a = venueIdentityForUrl('https://github.com/jqueryscript/awesome-claude-code');
  const b = venueIdentityForUrl('https://github.com/ccplugins/awesome-claude-code-plugins/blob/main/README.md');
  assert.equal(a.id, 'github-jqueryscript-awesome-claude-code');
  assert.equal(b.id, 'github-ccplugins-awesome-claude-code-plugins');
  assert.equal(a.name, 'github.com/jqueryscript/awesome-claude-code');
  assert.equal(venueIdentityForUrl('https://www.reddit.com/r/SideProject/submit').id, 'reddit-r-sideproject');
  assert.equal(venueIdentityForUrl('https://github.com/').id, 'github-com'); // bare host falls back
});
