...
test('setOpportunityStatus stamps posted_at once', () => {
  ...
  assert.equal(r.afterSkip, r.first);
});

test('goals CLI: seed once, log with idempotent ref, list, rm removes entries', () => {
  const store = makeStore();
  cli(store, ['config', 'init']);
  assert.match(cli(store, ['goals', 'seed']), /Added 8 starter goals/);
  ...
  assert.equal(list.goals.length, 8);
  ...
  assert.equal(list.totals.target, 23);
  ...
  assert.equal(after.goals.length, 7);
