...
// On macOS, Home/End (and their Shift/Control variants) scroll the page in
// Chrome instead of moving the caret, so a "select to line start" sequence
// silently lands in the wrong place. Inside an editable element the caret
// equivalents are Meta+Arrow keys; outside one Meta+ArrowLeft is browser Back,
// so the remap is only applied when the focused element is editable.
const MAC_CARET_KEYS = {
  'Home': 'Meta+ArrowLeft', 'End': 'Meta+ArrowRight',
  'Shift+Home': 'Shift+Meta+ArrowLeft', 'Shift+End': 'Shift+Meta+ArrowRight',
  'Control+Home': 'Meta+ArrowUp', 'Control+End': 'Meta+ArrowDown',
  'Shift+Control+Home': 'Shift+Meta+ArrowUp', 'Shift+Control+End': 'Shift+Meta+ArrowDown',
  'Control+Shift+Home': 'Shift+Meta+ArrowUp', 'Control+Shift+End': 'Shift+Meta+ArrowDown',
};
function macCaretKey(key) { return isMac ? (MAC_CARET_KEYS[key] || null) : null; }
...
const SOURCE_FILES = ['server.js', 'browser.js', 'open-link.js', 'ocr.js'].map(f => path.join(path.dirname(new URL(import.meta.url).pathname), f));
...
async function startMcpServer() {
  ...
  server.tool('open_link',
    'Optional alternative to click(ref) for a link: activate an <a> already on the page by its exact absolute HTTP(S) href (as shown by read_page). Useful when a call needs to carry the destination URL explicitly. Stricter than click: it only matches a visible anchor with that exact URL and refuses downloads, disabled links, links inside buttons, and target=_blank links (use open_tab for those). click(ref) remains the normal way to follow links. Returns requested and current URL.',
    { url: z.string().url().describe('Exact absolute HTTP(S) href of the conversation/navigation link shown by read_page') },
    async ({ url }) => {
      try { await ensure(); return ok(await browser.openLink(url)); }
      catch (e) { return err(e); }
    });
...
Use the refs with click("e3") / type("e3", text) / act. Refs are bound to the element (data-sc-ref), so they survive React re-renders and only reset when the page navigates; a ref that vanished fails fast with a hint. Nodes that belong to the site's global search UI are tagged [site-search] — never type into or click those when filling a form (a form's own typeahead is NOT tagged). Native date inputs show as one textbox [type=date] — act fill / set_value it with "YYYY-MM-DD".
Options: interactive (only clickable elements), compact (drop unnamed structure), maxDepth. Leave maxDepth alone unless you have a reason — the default (40) is not a token cost, depth counts only the levels you see printed, and a low cap silently drops page content (you would see a sidebar and a "Loading…" spinner but no posts).`,
    {
      ...
      maxDepth: z.number().optional().describe('Maximum depth of the PRINTED tree (default 40; unnamed wrapper nodes do not count). Rarely worth setting — too low drops content. Some sites floor it.'),
    },
    async ({ interactive, compact, maxDepth }) => {
      try {
        await ensure();
        const nodes = await browser.getAccessibilityTree({ interactive: !!interactive, compact: !!compact, maxDepth });
  ...
  server.tool('type_text',
    ...
    async ({ text, clear }) => {
      try {
        ...
        // Rich-text composers (Reddit's Lexical editor, X, Threads) swallow "\n"
        // inside one insertText and produce a single block; press Enter per line
        // there so the result matches the text. Plain textareas keep the paste.
        if (text.includes('\n') && await browser.activeElementIsContentEditable()) {
          await browser.insertLines(page, text, 'literal');
        } else {
  ...
  server.tool('insert_html',
    'Fill a RICH-TEXT editor (contenteditable: Indie Hackers product posts, Medium-style, Lexical, ProseMirror, TipTap) with formatted content in one shot — the reliable way to get real paragraphs, headings, bold, links and lists into an editor that does not render markdown. Pass either `markdown` (converted here: #/## headings, paragraphs, **bold**, *italic*, `code`, [links](url), ![images](url), lists, > quotes, fences, ---) or ready `html`. It focuses the editor (target, or the element you last clicked), replaces its content by default, and inserts via execCommand(\'insertHTML\') so the editor treats it as a paste (its own sanitizer applies; what it cannot represent is flattened). Set headings:"bold" for editors whose toolbar has no heading style (Indie Hackers) so "## Title" becomes a bold line instead of vanishing. Returns a structural summary (paragraphs/headings/strong/lists/links/images, text length) — verify with read_page/screenshot before the review stop. Large bodies are fine. NOT for plain <textarea>/<input> (use set_value/type there; markdown textareas take markdown as-is).',
    {
      markdown: z.string().optional().describe('Markdown body (converted to HTML here). Give markdown OR html.'),
      html: z.string().optional().describe('Ready HTML fragment (<p>, <h2>, <strong>, <a>, <ul>…). Give markdown OR html.'),
      target: z.string().optional().describe('Ref / CSS selector / label of the editor. Default: the currently focused element (click the editor first).'),
      replace: z.boolean().optional().describe('Replace existing editor content (default true). false = append at the end.'),
      headings: z.enum(['native', 'bold']).optional().describe('markdown only: "native" = <h1>-<h6> (default); "bold" = <p><strong>…</strong></p> for editors without heading styles.'),
    },
    async ({ markdown, html, target, replace, headings }) => {
      try {
        await ensure();
        if ((markdown == null) === (html == null)) throw new Error('pass exactly one of markdown or html');
        const fragment = html != null ? html : markdownToHtml(markdown, { headings: headings || 'native' });
        if (!fragment.trim()) throw new Error('nothing to insert (empty body)');
        const r = await browser.insertHtml(fragment, { target, replace: replace !== false });
        return ok({ ...r, source: html != null ? 'html' : 'markdown', htmlLength: fragment.length,
          message: `Inserted ${r.paragraphs} paragraph(s), ${r.headings} heading(s), ${r.strong} bold, ${r.lists} list(s), ${r.links} link(s) via ${r.method}` });
      } catch (e) { return err(e); }
    });
  ...
  server.tool('press_key',
    'Press a key or combination. Examples: Enter, Tab, Escape, ArrowDown, Control+a. On Mac, Control+a/c/v/x/z auto-convert to Meta, and inside an editable field Home/End (+Shift/Control) are remapped to the Mac caret keys (Meta+Arrow) because Chrome on macOS scrolls on Home/End instead of moving the caret. To collapse a selection to its end, press ArrowRight (never End).',
    { key: z.string() },
    async ({ key }) => {
      try {
        await ensure();
        let k = normalizeKeys(key);
        let note = '';
        const caret = macCaretKey(k);
        if (caret && await browser.activeElementIsEditable()) { note = ` (remapped from ${k}: Mac caret key)`; k = caret; }
        return ok({ message: (await browser.pressKey(k)) + note });
      } catch (e) { return err(e); }
    });
  ...
  server.tool('upload',
    'Upload local file(s) (for the posting flow — images/video). target is a ref from read_page (e.g. "e3") or a CSS selector: either the <input type=file> itself (make it specific when a page has several, input[name=screenshot]) OR the site\'s "Upload image" / "Choose file" button when no file input exists in the DOM — the tool clicks it, catches the file chooser, and hands over the files without any dialog appearing. Paths must be absolute. Use either this OR a site\'s "upload by URL" field for a given image, never both.',
    { target: z.string().describe('Ref (e.g. e3) or CSS selector of the file input, or of the upload button that opens the file chooser'), files: z.array(z.string()).describe('Absolute file paths to upload') },
  ...
  server.tool('collect_lead',
    "Record a person worth a personal invitation (Pro founder-invitations; only when the brief has a FOUNDER INVITATIONS section). Call it AFTER collect_opportunity for the same post and after investigating the author in a second tab. Pass an honest fit score with short quoted evidence and a DM draft (≤80 words, quotes their post, names their product, states the brand's requirement up front, one yes/no ask, no promises). Nothing is sent — the user reviews it in the Invitations tab. Skips people investigated in the last 90 days.",
    {
      url: z.string().describe("The post's direct permalink (same URL as the collect_opportunity call)"),
      platform: z.string().describe('Platform name, e.g. reddit, threads, indiehackers, x'),
      handle: z.string().describe('Their handle as shown, e.g. "u/name", "@name"'),
      profileUrl: z.string().describe('Their profile page URL'),
      brandId: z.string().describe('The one brand this invitation is for (name or id)'),
      ask: z.string().max(400).describe('Their own words that make this an explicit ask (quote)'),
      fitScore: z.number().min(1).max(10).describe('Honest 1-10 fit against the brand\'s ideal customer'),
      fitSummary: z.string().max(300).describe('One plain sentence on why they fit (or barely do)'),
      evidence: z.array(z.string().max(200)).max(8).describe('2-6 short facts you actually saw on their profile/posts/site'),
      draftDm: z.string().max(1200).describe('The invitation DM draft'),
      productName: z.string().optional().describe('What they built, if found'),
      productUrl: z.string().optional().describe("Their product's URL, if found"),
      disqualifiers: z.string().optional().describe('Anything arguing against reaching out (e.g. "Reddit: ask permission publicly first"), or omit'),
      runId: z.string().optional().describe('The discovery run id'),
    },
    async ({ url, platform, handle, profileUrl, brandId, ask, fitScore, fitSummary, evidence, draftDm, productName, productUrl, disqualifiers, runId }) => {
      try {
        if (!isPro(loadConfig())) {
          return ok({ skipped: true, reason: 'pro_required', message: 'Founder invitations are a Pro feature — skip lead collection this run.' });
        }
        if (db.isPersonSeen(profileUrl, { days: 90 })) {
          return ok({ skipped: true, reason: 'already_investigated', message: 'This person was investigated in the last 90 days — skipped.', profileUrl });
        }
        const brand = resolveBrand(brandId);
        if (!brand) throw new Error(`No brand matches "${brandId}"`);
        const opp = db.findOpportunityByUrl(url);
        const leadId = randomUUID();

        // Profile screenshot for the reviewer (the active tab should be the
        // profile when this is called). Non-fatal, same as collect_opportunity.
        let screenshotPath = null;
        try {
          await ensure();
          const dir = path.join(paths.runsDir(), runId || 'adhoc');
          fs.mkdirSync(dir, { recursive: true });
          const shotPath = path.join(dir, `lead-${leadId}.png`);
          fs.writeFileSync(shotPath, await browser.screenshotPng());
          screenshotPath = shotPath;
        } catch (e) {
          log(`[collect_lead] screenshot failed: ${e.message}`);
        }
        ...
        db.addLead({
          ...
          draftDm,
          status: opp?.status === 'posted' ? 'replied' : 'candidate',
          screenshotPath,
        });
        return ok({
          leadId,
          oppId: opp?.id ?? null,
          message: opp
            ? `Recorded lead ${handle} on ${platform} for ${brand.name} (linked to the collected reply)`
            : `Recorded lead ${handle} on ${platform} for ${brand.name} — no collected reply found for that URL; collect the public reply too`,
        });
      } catch (e) { return err(e); }
    });

  server.tool('record_performance',
    "Record how something the user posted is being received (the Pro performance check-in). For a reply (kind 'reply' in perf due): visit the thread, find the user's own comment, pass its score and direct-reply count. For a product post (kind 'product_post': a meme on a subreddit, a directory listing, a Show HN): the URL is the user's own post — pass the post's score/upvotes and its comment count. Omit a number if the platform doesn't show it. Note 'removed' if the post or comment is gone.",
    {
      oppId: z.string().describe('The id from "perf due" — an opportunity id (reply) or a product post id'),
      upvotes: z.number().optional().describe("Current score/upvotes on the user's comment or post"),
      replyCount: z.number().optional().describe("Direct replies to the user's comment, or comments on the user's post"),
      note: z.string().optional().describe('One-line color, e.g. "top reply, OP thanked them"'),
    },
    async ({ oppId, upvotes, replyCount, note }) => {
      try {
        const kind = db.getOpportunityById(oppId) ? 'reply' : (db.getPostById(oppId) ? 'product_post' : null);
        if (!kind) throw new Error(`No opportunity or product post with id ${oppId}`);
        db.recordReplyCheck(oppId, {
          upvotes: upvotes ?? null,
          replyCount: replyCount ?? null,
          note: note || '',
        });
        return ok({ message: `Recorded performance check for ${kind} ${oppId}`, oppId, kind });
      } catch (e) { return err(e); }
    });
