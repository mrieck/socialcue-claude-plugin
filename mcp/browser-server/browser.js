...
import { openLinkOnPage } from './open-link.js';
import {
  DEFAULT_CDP_URL,
  DEFAULT_PROFILE_DIR,
  probeCdp,
...
export { probeCdp, launchDedicatedChrome, fillNative, insertLines };

/**
 * Is the focused element a contenteditable? Walks into open shadow roots
 * (Reddit's composer lives inside shreddit custom elements, where
 * document.activeElement only reports the host).
 */
/**
 * Is the focused element editable at all (input / textarea / contenteditable,
 * not readonly or disabled)? Used to decide whether a caret key like Home/End
 * should be remapped on Mac — outside an editor Meta+ArrowLeft is browser Back.
 */
export async function activeElementIsEditable() {
  const p = getPage();
  return p.evaluate(() => {
...
/**
 * Replace (or append to) the content of a rich-text editor with HTML in one
 * shot. Runs inside the page: focuses the contenteditable (target, or the
 * active element — walking shadow roots), selects its contents when
 * `replace`, and uses execCommand('insertHTML') so the editor sees a normal
 * paste (undo stack, input events, its own sanitizer). Falls back to a Range
 * insert when execCommand is refused. Returns a small structural summary so
 * the caller can verify headings/paragraphs landed without a screenshot.
 */
export async function insertHtml(html, { target, replace = true } = {}) {
  const p = getPage();
  const fn = (el, { html, replace }) => {
    if (!el) {
      el = document.activeElement;
      while (el?.shadowRoot?.activeElement) el = el.shadowRoot.activeElement;
    }
    if (!el) throw new Error('no focused element — click the editor first or pass target');
    // a target may be the editor's wrapper; descend to the contenteditable
    if (!el.isContentEditable) {
      const inner = el.querySelector?.('[contenteditable="true"], [contenteditable=""]');
      if (inner) el = inner;
    }
    if (!el.isContentEditable) throw new Error(`<${el.tagName.toLowerCase()}> is not contenteditable — use set_value / type for plain inputs`);
    el.focus();
    const sel = (el.getRootNode?.().getSelection?.() || window.getSelection());
    const range = document.createRange();
    let ok = false;
    if (replace) {
      // select everything and paste over it — the editor sees one paste op
      ...
      if (!ok) {
        ...
        r.deleteContents(); r.insertNode(tpl.content);
      }
    } else {
      // append: execCommand would normalise the caret INTO the last block
      // (a <p> pasted at the end of a list lands inside its last <li>), so
      // add the new blocks as siblings after the last child instead.
      const tpl = document.createElement('template'); tpl.innerHTML = html;
      el.append(tpl.content);
      range.selectNodeContents(el); range.collapse(false);
      sel.removeAllRanges(); sel.addRange(range);
    }
    try { el.dispatchEvent(new InputEvent('input', { bubbles: true, inputType: 'insertFromPaste', data: null })); }
    catch { el.dispatchEvent(new Event('input', { bubbles: true })); }
    const count = q => el.querySelectorAll(q).length;
    const text = el.textContent || '';
    return {
      method: ok ? 'execCommand' : 'range',
      paragraphs: count('p'), headings: count('h1,h2,h3,h4,h5,h6'), strong: count('strong,b'),
      lists: count('ul,ol'), links: count('a[href]'), images: count('img'),
      textLength: text.length, sample: text.slice(0, 120),
    };
  };
  if (target) {
    const locator = await resolveTarget(target);
    return locator.evaluate(fn, { html, replace });
  }
  // page.evaluate hands the function a single argument; call the same body
  // with el=null so it resolves the active element itself.
  return p.evaluate(`(${fn.toString()})(null, ${JSON.stringify({ html, replace })})`);
}

export async function activeElementIsContentEditable() {
  const p = getPage();
  return p.evaluate(() => {
    let el = document.activeElement;
    while (el?.shadowRoot?.activeElement) el = el.shadowRoot.activeElement;
...
/**
 * Collapse the shadow parts of each native date/time input (spinbuttons
 * "Month"/"Day"/"Year" + "Show date picker") into ONE textbox node bound to the
 * input element itself, so `act fill` / `set_value` with "YYYY-MM-DD" just
 * works. `act fill` on a lone spinbutton part times out — that is what this
 * replaces.
 */
function collapseDateInputs(results, domInfo) {
  const seen = new Map(); // input bid -> merged node
  const out = [];
  for (const node of results) {
    const d = domInfo.get(node.backendNodeId);
    const owner = d?.dateOf;
    if (!owner) { out.push(node); continue; }
    let merged = seen.get(owner.bid);
    if (!merged) {
      merged = { role: 'textbox', name: owner.label, value: '', backendNodeId: owner.bid, inputType: owner.type, parts: {} };
      seen.set(owner.bid, merged);
      out.push(merged);
    }
    if (node.role === 'spinbutton') merged.parts[node.name.toLowerCase()] = node.value;
  }
  for (const m of seen.values()) {
    const { year, month, day, hours, minutes } = m.parts;
    const n = v => (v == null || v === '' || Number(v) === 0) ? null : String(v).padStart(2, '0');
    if (m.inputType === 'date' && n(year) && n(month) && n(day)) m.value = `${year}-${n(month)}-${n(day)}`;
    else if (m.inputType === 'month' && n(year) && n(month)) m.value = `${year}-${n(month)}`;
    else if (m.inputType === 'time' && n(hours) != null && n(minutes) != null) m.value = `${n(hours)}:${n(minutes)}`;
    delete m.parts;
  }
  return out;
}
...
async function domFacts(client, nodes) {
  ...
  while (stack.length) {
    ...
    const next = {
      ...
      // A native date/time input renders as shadow spinbuttons (Month/Day/Year)
      // plus a picker button; remember which input they belong to so read_page
      // can collapse them into one fillable field.
      dateInput: ctx.dateInput || (name === 'input' && DATE_INPUT_TYPES.has(type)
        ? { bid: n.backendNodeId, type, label: a['aria-label'] || a.name || a.id || '' } : null),
    };
    if (n.nodeType === 1 && wanted.has(n.backendNodeId)) {
      info.set(n.backendNodeId, {
        ...
        // "Site search" = the site's global search UI, not any <input type=search>:
        // a form's own typeahead (SaaSHub's feature/platform pickers are
        // type=search inside a regular <form>) must stay fillable.
        inSearch: next.search || (next.chrome && searchy(name, a)) || (type === 'search' && !next.form),
        dateOf: next.dateInput && next.dateInput.bid !== n.backendNodeId ? next.dateInput : null,
      });
    }
...
/**
 * Per-site floor for read_page's maxDepth, keyed by hostname suffix. The agent
 * sometimes passes a small maxDepth to save tokens; on these sites that used to
 * silently drop the content it came for (Threads feeds/search came back as a
 * sidebar plus a "Loading..." spinner). Depth is now counted over emitted nodes
 * so this is belt-and-braces, but it keeps a too-small cap from ever biting on
 * a site whose DOM is unusually deep.
 */
const SITE_MIN_DEPTH = {
  'threads.com': DEFAULT_MAX_DEPTH,
};

export function effectiveMaxDepth(requested, url) {
  let depth = Number.isFinite(requested) && requested > 0 ? requested : DEFAULT_MAX_DEPTH;
  let host = '';
  try { host = new URL(url || '').hostname.toLowerCase(); } catch { /* no page yet */ }
  for (const [suffix, floor] of Object.entries(SITE_MIN_DEPTH)) {
    if (host === suffix || host.endsWith(`.${suffix}`)) depth = Math.max(depth, floor);
  }
  return depth;
}

/**
 * Turn the flat CDP AX node list into the ordered list of nodes read_page
 * prints. Pure — no page access — so it can be unit-tested against synthetic
 * trees.
 *
 * `depth` counts EMITTED ancestors, not raw AX ancestors. Chrome's raw tree
 * wraps content in dozens of ignored/unnamed nodes (Threads nests every post
 * 32+ raw levels down), so a raw-depth cap dropped exactly the content the
 * agent asked for while keeping shallow chrome like the sidebar and the
 * infinite-scroll spinner. Counting only what the agent actually sees makes
 * maxDepth mean "levels of the tree you get back".
 */
export function walkAXNodes(axNodes, options = {}) {
  const { maxNodes = 500, interactive = false, compact = false, maxDepth = DEFAULT_MAX_DEPTH } = options;
  ...
  while (stack.length > 0 && results.length < maxNodes) {
    ...
    // Logical depth only grows on emitted nodes, so once a node is past the
    // cap its whole subtree is too — no need to walk it.
    if (depth > maxDepth) continue;

    // Everything below decides whether THIS node is printed. A skipped node is
    // transparent: its children inherit its depth instead of depth + 1.
    const skip = () => pushChildren(node, depth);

    if (node.ignored) { skip(); continue; }
    // InlineTextBox is a text-run duplicate of its parent's text — pure noise.
    if (role === 'InlineTextBox') { skip(); continue; }
    if (compact && LAYOUT_ROLES.has(role)) { skip(); continue; }
    ...
    if (interactive && !INTERACTIVE_ROLES.has(role)) { skip(); continue; }
    if (compact && STRUCTURAL_ROLES.has(role) && !name) { skip(); continue; }
    const isInteractiveRole = INTERACTIVE_ROLES.has(role);
    if (!name && !value && !isInteractiveRole) { skip(); continue; }
    // A11y trees repeat a named node's text as StaticText children; drop echoes.
    if (compact && role === 'StaticText' && seenText.has(name)) { skip(); continue; }
    ...
    if (compact && name) {
      ...
      if (seenExact.has(exactKey)) { skip(); continue; }
...
export async function getAccessibilityTree(options = {}) {
  ...
  try {
    ...
    const results = walkAXNodes(axNodes, {
      maxNodes, interactive, compact,
      maxDepth: effectiveMaxDepth(maxDepth, p.url()),
    });
    ...
    const collapsed = collapseDateInputs(results, domInfo);

    refMap = buildRefMap(collapsed, domInfo, p);
    return collapsed;
  } finally {
    await client.detach().catch(() => {});
  }
...
export function formatTreeAsText(nodes) {
  ...
  for (const node of nodes) {
    ...
    if (node.inputType) attrs.push(`type=${node.inputType}`, node.inputType === 'time' ? 'fill "HH:MM"' : node.inputType === 'month' ? 'fill "YYYY-MM"' : 'fill "YYYY-MM-DD"');
...
/**
 * Optional URL-addressed link activation (the `open_link` tool). NOT on the
 * click(ref) path: ref clicks stay real, trusted mouse clicks resolved by
 * backendNodeId. This exists for clients whose approval layer needs the
 * destination URL in the call itself.
 */
export async function openLink(url) {
  return openLinkOnPage(getPage(), url);
}
...
// ============================================================================
/**
 * Upload local file(s). `target` is a ref (from read_page) or a CSS selector /
 * label. Paths must be absolute and exist. Three strategies, cheapest first:
 *   1. target IS an <input type=file>            -> setInputFiles on it.
 *   2. target is a button/label/drop-zone that   -> click it while listening for
 *      opens the OS file chooser (no <input>        Playwright's `filechooser`
 *      in the DOM until clicked, e.g. Indie         event and hand the files to
 *      Hackers "Upload Image", Filestack)           the chooser.
 *   3. the click created a fresh <input type=file> -> setInputFiles on that.
 * Strategy 2/3 also kick in when a CSS selector for an <input type=file> does
 * not match anything yet: we click the nearest upload-ish button instead of
 * hanging for 30s on a locator that never appears.
 */
const FILE_INPUT_TIMEOUT_MS = 2500;
const FILE_CHOOSER_TIMEOUT_MS = 8000;

async function isFileInput(locator) {
  try {
    return await locator.evaluate((el) => el instanceof HTMLInputElement && el.type === 'file', { timeout: FILE_INPUT_TIMEOUT_MS });
  } catch { return false; }
}

async function findUploadTrigger(p) {
  // Something that looks like an upload control: a button/label whose text or
  // aria-label mentions upload/choose/browse/image/photo/logo/file, else a
  // label wired to a file input.
  const rx = /upload|choose|browse|attach|add (an? )?(image|photo|logo|file|screenshot)|select (an? )?(image|photo|file)/i;
  const candidates = p.locator('button, label, [role=button], a[href="#"], div[tabindex]');
  const n = await candidates.count();
  for (let i = 0; i < Math.min(n, 200); i++) {
    const c = candidates.nth(i);
    const text = ((await c.getAttribute('aria-label')) || (await c.innerText().catch(() => '')) || '').trim();
    if (rx.test(text) && await c.isVisible().catch(() => false)) return c;
  }
  return null;
}
...
export async function uploadFile(target, files) {
  ...
  const p = getPage();
  const ref = isRef(target);
  let locator = ref ? await resolveRef(target) : (isSelector(target) ? p.locator(target).first() : await resolveTarget(target));

  // Strategy 1: the target itself is a file input that exists right now.
  if (await locator.count().catch(() => 0) > 0 && await isFileInput(locator)) {
    await locator.setInputFiles(list);
    return `Uploaded ${list.length} file(s) to ${target}`;
  }

  // The target is not (yet) a file input. Decide what to click: the target if
  // it exists and is visible (a button/label), else an upload-looking control.
  let trigger = null;
  if (await locator.count().catch(() => 0) > 0 && await locator.isVisible().catch(() => false)) trigger = locator;
  else trigger = await findUploadTrigger(p);
  if (!trigger) throw new Error(`No file input matches "${target}" and no upload button was found on the page. Pass the ref of the site's upload button instead.`);

  // Strategy 2: click while listening for the file chooser. Playwright
  // intercepts the native dialog, so nothing pops up on the user's screen.
  const chooserPromise = p.waitForEvent('filechooser', { timeout: FILE_CHOOSER_TIMEOUT_MS }).catch(() => null);
  await trigger.click({ timeout: 5000 });
  const chooser = await chooserPromise;
  if (chooser) {
    await chooser.setFiles(list);
    await p.waitForTimeout(randomDelay(300, 600));
    return `Uploaded ${list.length} file(s) via the file chooser opened by ${target}`;
  }

  // Strategy 3: the click injected a file input into the DOM (Ember/Filestack
  // style) without opening a chooser we caught — use the newest one.
  const inputs = p.locator('input[type=file]');
  const count = await inputs.count();
  if (count > 0) {
    await inputs.nth(count - 1).setInputFiles(list);
    return `Uploaded ${list.length} file(s) to the file input created by clicking ${target}`;
  }
  throw new Error(`Clicked ${target} but no file chooser opened and no <input type=file> appeared. Try the ref of the actual upload button, or screenshot to see what the page did.`);
}
