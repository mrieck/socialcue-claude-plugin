...
/**
 * Slug + display name for a destination given only its URL. Plain sites slug
 * by hostname (`saashub.com` → `saashub-com`), but multi-tenant hosts hold
 * thousands of distinct venues — every awesome-list on GitHub, every
 * subreddit — so those keep the first path segments:
 * `github.com/jqueryscript/awesome-claude-code` → `github-jqueryscript-awesome-claude-code`.
 */
export const MULTI_TENANT_HOSTS = new Set(['github.com', 'gitlab.com', 'codeberg.org', 'reddit.com', 'old.reddit.com', 'x.com', 'twitter.com', 'notion.site', 'substack.com']);
export function venueIdentityForUrl(url) {
  const u = new URL(url);
  ...
  if (MULTI_TENANT_HOSTS.has(host)) {
    ...
    if (segs.length) {
      const base = host.replace(/\.(com|org|site)$/, '');
      return { host, id: slug(`${base}-${segs.join('-')}`), name: `${host}/${segs.join('/')}`, multiTenant: true };
    }
  }
  return { host, id: slug(host), name: host, multiTenant: false };
}
