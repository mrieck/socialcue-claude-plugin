...
const HANDLERS = {
  ...
  async goals(positional, flags) {
    ...
    if (sub === 'list' || sub === undefined) {
      ...
      out(`Week ${summary.week.label}${summary.week.isCurrent ? ' (this week)' : ''}`);
      for (const g of summary.goals) {
        const mark = g.count >= g.target ? '✓' : ' ';
        const detail = `${g.autoCount} auto + ${g.manualCount} logged`;
    ...
    if (sub === 'log') {
      ...
      const week = goals.weekBounds(new Date(r.entry.at));
    ...
    die('Unknown goals subcommand. Use: list [--week YYYY-MM-DD --json] | add <json|file|-> | log <id|title> [--note s --at ISO --ref key --source name] | rm <id> | seed');
  },
