...
/**
 * Maintainer machines (venues.adminKey set) may flip `devPreviewFree` to see
 * exactly what a free install sees. Ignored everywhere else, so a stray flag
 * in someone's config can't do anything.
 */
export function isPreviewingFree(config) {
  return config.devPreviewFree === true && !!config.venues?.adminKey;
}

/** Is this config Pro-licensed (signed, right tier, not expired)? */
export function isPro(config) {
  if (isPreviewingFree(config)) return false;
...
export function licenseInfo(config) {
  const dev = config.venues?.adminKey ? { canPreviewFree: true, previewFree: config.devPreviewFree === true } : {};
  if (!config.license) return { pro: false, ...dev };
  const parsed = parseLicense(config.license);
  if (!parsed.valid) {
    // Surface an expired-but-genuine key so the UI can nudge a reconnect.
    return { pro: false, expired: parsed.expired === true, ...dev };
  }
  if (isPreviewingFree(config)) {
    // The key is fine; we're deliberately looking at the free tier.
    return { pro: false, ...dev, email: parsed.payload.email };
  }
  return {
    pro: true,
