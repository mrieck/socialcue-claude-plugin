 ...
 * required for every channel; X additionally validates who_can_reply_post.
 */
function defaultSettingsFor(identifier) {
  if (!identifier) return {};
  const base = { __type: identifier };
  if (identifier === 'x' || identifier === 'twitter') base.who_can_reply_post = 'everyone';
  return base;
}

/**
 * Build the Postiz create-post payload from a content_items row (parsed:
 * channels as array, settings as object). Pure — exported for CLI reuse.
 * One call carries every selected channel, which keeps us far under the
 * ~90 req/hour create limit.
 *
 ...
 * whatever the editor collected in item.settings[integrationId] wins on top.
 */
export function buildPostPayload(item, { type = 'schedule', date, images = [], identifierById = {} } = {}) {
  return {
    ...
    posts: (item.channels ?? []).map(id => ({
      ...
      value: [{ content: item.body ?? '', image: images }],
      settings: { ...defaultSettingsFor(identifierById[id]), ...(item.settings?.[id] ?? {}) },
    })),
  };
}
