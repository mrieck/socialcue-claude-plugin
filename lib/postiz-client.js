 ...
 */
import { tagLinks, utmSourceFor } from './utm.js';
...
function tagContent(text, identifier, utm) {
  if (!utm) return text;
  return tagLinks(text, { ...utm, source: utmSourceFor(identifier) });
}
 ...
 *
 * `utm` = { campaign, content, ownHosts } (or null when config.utmTagging is
 * off) appends utm_* params to own-site links in each channel's text, with
 * utm_source derived from that channel's platform — see lib/utm.js.
 */
export function buildPostPayload(item, { type = 'schedule', date, images = [], identifierById = {}, utm = null } = {}) {
  return {
    type,
    ...
    posts: (item.channels ?? []).map(id => {
      ...
      return {
        ...
        value: [{ content: tagContent(variant.content ?? item.body ?? '', identifierById[id], utm), image: images }],
        settings: {
