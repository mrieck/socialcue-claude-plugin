  ...
  db.prepare(
    `INSERT INTO content_items
       (id, brand_id, brand_name, title, body, channels, settings, status,
        scheduled_for, venue_id, media, source, variants, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
  ).run(
    id,
    item.brandId ?? null,
...
export function updateContentItem(
  id,
  { brandId, brandName, title, body, channels, settings, status, scheduledFor, venueId, media, variants } = {}
) {
