 ...
 *
 * Weeks run from 00:00 local on the configured start day (config.weekStartsOn:
 * 0 = Sunday, the default; 1 = Monday) to 00:00 local seven days later,
 ...
 *
 * An auto-counted item the user disagrees with ("that TikTok was last week's")
 * can be excluded per goal (goal_exclusions); the count and the sidebar badge
 * both honour that.
 */
...
export function configuredWeekStart() {
  try {
    const n = Number(cfg.loadConfig().weekStartsOn);
 ...
 * last day of the week, for display); `label` reads "Sep 6 – Sep 12" and
 * `labelLong` "Sun Sep 6 – Sat Sep 12". `daysLeft` counts the days still to
 * come in the current week (today included; 0 for other weeks) and `endsAt`
 * is the exact instant the week rolls over.
 */
export function weekBounds(date = new Date(), weekStartsOn = 0) {
  ...
  const isCurrent = today >= d && today < end;
  const todayStart = new Date(today);
  todayStart.setHours(0, 0, 0, 0);
  const daysLeft = isCurrent ? Math.round((end - todayStart) / 86400000) : 0;
  return {
    start: d.toISOString(),
    end: end.toISOString(),
    ...
    labelLong: `${DAYS[d.getDay()]} ${fmt(d)} – ${DAYS[last.getDay()]} ${fmt(last)}`,
    isCurrent,
    daysLeft,
  };
}
...
export const RULE_TYPES = ['content', 'post', 'reply', 'invitation'];
...
export function validateRule(rule) {
  ...
  // reply | invitation — both are "count per platform" rules
  const platform = optStr(rule.platform, 'platform', { lower: true });
...
/**
 * The rows one rule matches inside a week, oldest first:
 * `{ kind, id, title, at }` (+ `status` for content/posts). Manual-only → [].
 * `kind` is the rule type, so an exclusion key is `${kind}:${id}`.
 */
export function matchRule(rule, brandId, week) {
  const r = parseRule(rule);
  if (!r) return [];
  const { start, end } = week;
  if (r.type === 'content') {
    return db.listContentInWeek({
  ...
  if (r.type === 'post') {
    return db.listPostsInWeek({ start, end, brandId: brandId ?? null, postType: r.postType, destinationHost: r.destinationHost });
  }
  if (r.type === 'invitation') {
    return db.listInvitedInWeek({
      start, end, brandId: brandId ?? null,
      platformKeys: r.platform ? platformAliases(r.platform) : null,
    });
  }
  return db.listRepliesInWeek({
    start, end, brandId: brandId ?? null,
    platformKeys: r.platform ? platformAliases(r.platform) : null,
  });
}

/** Auto count for one rule inside a week, before exclusions. Manual-only goals → 0. */
export function evaluateRule(rule, brandId, week) {
  return matchRule(rule, brandId, week).length;
}

/** Matched items flagged with the goal's exclusions, plus the count that stands. */
export function autoItemsForGoal(row, week) {
  const items = matchRule(parseRule(row.rule), row.brand_id, week);
  if (!items.length) return { items, autoCount: 0, excludedCount: 0 };
  const excluded = db.listGoalExclusions(row.id);
  let excludedCount = 0;
  const flagged = items.map(it => {
    const isExcluded = excluded.has(`${it.kind}:${it.id}`);
    if (isExcluded) excludedCount += 1;
    return { ...it, excluded: isExcluded };
  });
  return { items: flagged, autoCount: flagged.length - excludedCount, excludedCount };
}
...
export function describeRule(rule) {
  ...
  if (r.type === 'content') {
    ...
    bits.push(r.type === 'invitation' ? 'invitations' : 'reply');
    if (r.platform) bits.push(r.platform);
  }
  return bits.join(' · ');
}
...
export function goalSummary(row, week, brands) {
  ...
  const { items: autoItems, autoCount, excludedCount } = autoItemsForGoal(row, week);
 ...
 */
export function weekSummary({ weekParam = null, date = null, includeArchived = false, weekStartsOn = null } = {}) {
  const anchor = parseWeekParam(weekParam) ?? date ?? new Date();
  const week = weekBounds(anchor, weekStartsOn ?? configuredWeekStart());
...
export function goalsShortCount() {
  ...
  const week = weekBounds(new Date(), configuredWeekStart());
  let short = 0;
  for (const row of db.listGoals()) {
    const auto = autoItemsForGoal(row, week).autoCount;
...
export const STARTER_GOALS = [
  ...
  { title: 'Founder invitations (matched, personal)', target: 5, rule: { type: 'invitation' } },
];
