...
import { openInDedicatedChrome } from './browser-core.js';
...
/**
 * Open the dashboard where the user's logins live: the dedicated Social Cue
 * Chrome profile (same one discovery drives), not the OS default browser.
 * Falls back to the default browser only when Chrome can't be found or the
 * dedicated profile refuses to open. Returns { where, note? }:
 *   where = 'socialcue-tab' | 'socialcue-launched' | 'default' | 'none'
 */
export async function openDashboard(url) {
  const browser = cfg.loadConfig().browser;
  let note = null;
  try {
    const r = await openInDedicatedChrome(url, {
      cdpUrl: browser.cdpUrl,
      ...(browser.profilePath ? { profileDir: browser.profilePath } : {}),
    });
    if (r?.how === 'tab') return { where: 'socialcue-tab' };
    if (r?.how === 'launched') return { where: 'socialcue-launched' };
    note = 'no Chrome/Chromium binary found';
  } catch (err) {
    note = String(err?.message ?? err);
  }
  const opened = await openInDefaultBrowser(url);
  return { where: opened ? 'default' : 'none', note };
}
