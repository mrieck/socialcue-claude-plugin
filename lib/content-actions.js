...
import { ownHostsFromBrands } from './utm.js';
 ...
 */
/**
 * Auto-UTM tagging (config.utmTagging, default on): links to the user's own
 * brand sites get utm_source (per channel) / utm_medium=social /
 * utm_campaign=<brand id> / utm_content=<content item id>. Null = off.
 */
function utmOptions(config, row) {
  if (config.utmTagging === false) return null;
  const ownHosts = ownHostsFromBrands(config.brands);
  if (!ownHosts.size) return null;
  return { campaign: row.brand_id || 'content', content: row.id, ownHosts };
}
...
export async function scheduleContentItem(config, id, { type = 'schedule', date } = {}) {
  ...
  // An empty shared body is fine as long as every selected channel has its own
  // variant caption to fall back on.
  const uncovered = item.channels.some(ch => !item.variants?.[ch]?.content?.trim());
  if (!item.body.trim() && uncovered) return { ok: false, code: 'empty_body', message: 'draft is empty' };
  ...
  const payload = buildPostPayload(
    {
      channels: item.channels, settings: item.settings, variants: item.variants,
      title: item.title, body: item.body, scheduledFor: when,
    },
    { type, date: when, images, identifierById, utm: utmOptions(config, row) }
  );
