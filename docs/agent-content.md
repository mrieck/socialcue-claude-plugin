...
- Optional `format` — `text` (default: a social caption, one body for every
  channel) or `markdown` (a finished article for a markdown editor — Indie
  Hackers, dev.to, Hashnode). Article producers set `markdown`; the
  `/socialcue-post` article flow then pastes the body as-is.
- Optional `notes` — one or two lines from you to whoever posts it: the venue
  it was written for, where images are hosted, anything to watch. Shown
  read-only in the dashboard and printed by `content list`.

### Article producers (Indie Hackers, dev.to…)

Those sites take markdown and reference images **by public URL** — Indie
Hackers cannot upload images at all. So an article producer hosts its images
somewhere public first and writes them into the body as `![alt](https://…)`,
then also passes the local files in `media` (hero first) so the poster can
fill a cover-image slot where a form has one. The seoblog plugin's
`/seoblog:socialcue` does exactly this (hosting on the user's own blog):

```json
{ "title": "How I shipped X on a Claude subscription",
  "body": "![hero](https://example.com/blog/images/42)\n\nFirst paragraph…\n\n## What happened\n…",
  "brandName": "Acme", "status": "draft", "source": "seoblog",
  "format": "markdown",
  "notes": "Written for Indie Hackers. Images hosted on example.com (asset draft #17). Hero = media[0].",
  "media": ["/abs/path/how-i-shipped-x-hero.jpg"] }
```
