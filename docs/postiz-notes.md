...
- **No reschedule via the public API** — UPDATE 2026-08-20: upstream now has
  `PUT /posts/:id/settings` (merged update) and `PUT /posts/:id/status`
  (`draft|schedule`). We still don't use them: delete + recreate stays the
  rule and keeps the local-mirror model simple.
- ~90 requests/hour (100 cloud) on create; one create call carries all
  ...
  (The OpenAPI blurb says 30/hour — stale; the real docs say 90/100.)
- Auth is the raw API key in `Authorization` (no `Bearer` prefix).
- Endpoints we use: `GET /integrations`, `POST /posts`, `GET /posts`,
  `DELETE /posts/:id`, `GET /analytics/post/:id`, `POST /upload`, and (since
  the crosspost flow) `GET /integration-settings/:id` — returns
  `{output: {rules, maxLength, settings, tools}}` where `settings` is a JSON
  Schema of the provider DTO (or a string when none). Still unused:
  `POST /integration-trigger/:id` (dynamic data like Reddit flairs, IG audio).

## Per-platform DTO facts (verified against postiz-app source, 2026-08-20)

- `posts[]` in `POST /posts` is per-integration: each entry has its own
  `value[].content` and `settings` — different captions per channel in one
  call is the intended shape. `__type` = the channel's `identifier`.
- **YouTube**: `title` REQUIRED (2–100), `type` REQUIRED
  (`public|private|unlisted`), `tags` `[{value,label}]` ≤500 chars total,
  optional `thumbnail` MediaDto, `selfDeclaredMadeForKids` `'yes'|'no'`.
  Exactly ONE attachment, must be video. Description ≤5000 chars. No Shorts
  flag — YouTube infers from vertical aspect + duration.
- **TikTok**: validator requires `privacy_level`, `duet`, `stitch`, `comment`,
  `autoAddMusic`, `brand_content_toggle`, `brand_organic_toggle`,
  `content_posting_method`. Use `DIRECT_POST` — `UPLOAD` only drops media in
  the app inbox while Postiz reports success. `video_made_with_ai` is the AI
  disclosure. Unaudited apps get privacy forced to `SELF_ONLY` (verify cloud
  with a draft on first use).
- **Instagram**: `post_type` REQUIRED, `'post'|'story'` — NO `'reel'` value; a
  single video with `'post'` becomes a Reel. Identifier may be `instagram` or
  `instagram-standalone` (standalone lacks `audio`).
- **Uploads**: multipart field `file`; images ≤10MB, video ≤1GB, `video/mp4`
  ONLY (no mov/webm); response `{id, path}` goes into `value[].image` (video
  uses the same array). Media must be Postiz-uploaded URLs. Inapplicable
  settings are silently DISCARDED, not rejected. Our `uploadFile` uses a 5-min
  timeout (the 20s create timeout would abort any real video).
