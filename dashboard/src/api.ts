 ...
 */
import type { AssetOnFile, Brand, BrandDetail, CanonicalStatus, ContentItem, ContentStatus, Counts, Destination, DestinationKind, DirectorySettings, Filters, Goal, GoalEntry, GoalPatch, GoalsBoard, Lead, LeadStatus, LicenseInfo, Opportunity, PerformanceRow, PostIntentResult, PostizChannel, PostizStatus, PostStatus, ProductPost, Settings } from './types';
...
/* ---------- Founder invitations (Pro) ---------- */

export function getLeads(params: { status?: string; brand?: string; platform?: string } = {}): Promise<{ leads: Lead[]; pro: boolean }> {
  const q = new URLSearchParams();
  for (const [k, v] of Object.entries(params)) if (v) q.set(k, v);
  const qs = q.toString();
  return api<{ leads: Lead[]; pro: boolean }>(`api/leads${qs ? `?${qs}` : ''}`);
}
...
export function patchLead(id: string, patch: LeadPatch): Promise<Lead> {
  return api<{ lead: Lead }>(`api/leads/${encodeURIComponent(id)}`, {
    method: 'PATCH',
    body: JSON.stringify(patch),
  }).then(r => r.lead);
}

/** Open the person's profile in the dedicated browser (the DM is on the clipboard already). */
export function openLead(id: string): Promise<{ opened: boolean; message: string; profileUrl: string }> {
  return api(`api/leads/${encodeURIComponent(id)}/open`, { method: 'POST', body: '{}' });
}
...
/** Maintainer-only: flip the free-tier preview (403 elsewhere). */
export function setPreviewFree(previewFree: boolean): Promise<LicenseInfo> {
  return api<LicenseInfo>('api/settings/dev', { method: 'PATCH', body: JSON.stringify({ previewFree }) });
}
...
/** The venue playbook (Pro). Free installs get an empty list with pro:false. */
export function getDestinations(kind?: DestinationKind): Promise<{ destinations: Destination[]; pro: boolean }> {
  const q = kind ? `?kind=${encodeURIComponent(kind)}` : '';
  return api<{ destinations: Destination[]; pro?: boolean }>(`api/destinations${q}`)
    .then(r => ({ destinations: r.destinations, pro: r.pro !== false }));
}

/** Post attempts (Pro). Throws ApiError(403 'pro_required') on free. */
export function getPosts(params: { brand?: string; subject?: string; kind?: string; status?: string } = {}): Promise<ProductPost[]> {
  const q = new URLSearchParams();
  for (const [k, v] of Object.entries(params)) if (v) q.set(k, v);
  const qs = q.toString();
  return api<{ posts: ProductPost[] }>(`api/posts${qs ? `?${qs}` : ''}`).then(r => r.posts);
}

/** Status-level edits only (mark live/skipped, paste the listing URL). */
export function patchPost(
  id: string,
  patch: { status?: PostStatus; listingUrl?: string },
): Promise<ProductPost> {
  return api<{ post: ProductPost }>(`api/posts/${encodeURIComponent(id)}`, {
    method: 'PATCH',
    body: JSON.stringify(patch),
  }).then(r => r.post);
}

/** Assets cached for a subject (free). */
export function getAssets(subjectKey: string): Promise<AssetOnFile[]> {
  return api<{ assets: AssetOnFile[] }>(`api/assets/${encodeURIComponent(subjectKey)}`).then(r => r.assets);
}

/** One cached asset as an object URL (auth'd fetch; caller revokes). */
export async function fetchAssetUrl(subjectKey: string, file: string): Promise<string> {
  const token = localStorage.getItem(TOKEN_KEY) ?? '';
  const res = await fetch(`api/assets/${encodeURIComponent(subjectKey)}/${encodeURIComponent(file)}`, {
    headers: { Authorization: `Bearer ${token}` },
  });
  if (!res.ok) throw new ApiError(res.status, res.statusText);
  return URL.createObjectURL(await res.blob());
...
/** Save the signup email / webmail URL used for new accounts (free — setup, not acting). */
...
/** The board for the week containing `week` (local YYYY-MM-DD); omit for this week. */
export function getGoals(week?: string | null, archived = false): Promise<GoalsBoard> {
  const q = new URLSearchParams();
  if (week) q.set('week', week);
  if (archived) q.set('archived', '1');
  const qs = q.toString();
  return api<GoalsBoard>(`api/goals${qs ? `?${qs}` : ''}`);
}

export function createGoal(patch: GoalPatch & { title: string; target: number }, week?: string | null): Promise<Goal> {
  return api<{ goal: Goal }>('api/goals', { method: 'POST', body: JSON.stringify({ ...patch, week: week ?? undefined }) }).then(r => r.goal);
}

export function patchGoal(id: string, patch: GoalPatch, week?: string | null): Promise<Goal> {
  return api<{ goal: Goal }>(`api/goals/${encodeURIComponent(id)}`, {
    method: 'PATCH', body: JSON.stringify({ ...patch, week: week ?? undefined }),
  }).then(r => r.goal);
}

export function deleteGoal(id: string): Promise<void> {
  return api<{ deleted: boolean }>(`api/goals/${encodeURIComponent(id)}`, { method: 'DELETE' }).then(() => undefined);
}

export function seedGoals(): Promise<{ added: number; skipped?: boolean }> {
  return api<{ added: number; skipped?: boolean }>('api/goals/seed', { method: 'POST', body: '{}' });
}

/** "Log one": a manual entry (defaults to now). Returns the refreshed goal. */
export function logGoalEntry(id: string, body: { at?: string; note?: string } = {}, week?: string | null): Promise<{ entry: GoalEntry; goal: Goal }> {
  return api<{ entry: GoalEntry; goal: Goal }>(`api/goals/${encodeURIComponent(id)}/entries`, {
    method: 'POST', body: JSON.stringify({ ...body, week: week ?? undefined }),
  });
}

/** "Don't count this one" / "count it again" for an auto-matched item. Returns the refreshed goal. */
export function setGoalItemExcluded(goalId: string, kind: string, itemId: string, excluded: boolean, week?: string | null): Promise<Goal> {
  return api<{ goal: Goal }>(`api/goals/${encodeURIComponent(goalId)}/items/${encodeURIComponent(kind)}/${encodeURIComponent(itemId)}`, {
    method: 'PATCH', body: JSON.stringify({ excluded, week: week ?? undefined }),
  }).then(r => r.goal);
}

export function deleteGoalEntry(goalId: string, entryId: string, week?: string | null): Promise<Goal> {
  const q = week ? `?week=${encodeURIComponent(week)}` : '';
  return api<{ goal: Goal }>(`api/goals/${encodeURIComponent(goalId)}/entries/${encodeURIComponent(entryId)}${q}`, { method: 'DELETE' }).then(r => r.goal);
}
