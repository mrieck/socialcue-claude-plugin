...
export interface Opportunity {
  ...
  /** When the reply was marked posted (any path); null on legacy rows. */
...
/** '' = newest first (bridge order); 'score' = by score inside each run, runs newest first. */
export type QueueSort = '' | 'score';
...
export interface Filters {
  ...
  sort: QueueSort;   // client-side only — never sent to the bridge
}

/** Sidebar badge counts (from /api/changes). */
export interface Counts {
  opportunities: number;   // new replies
  analytics: number;       // weekly goals still short this week
  content: number;         // content ideas/drafts + fresh "places to post" nudges
  invitations?: number;    // Pro: people found but not yet invited — shown pinned under Conversations (older bridges omit it)
}

export type View = 'opportunities' | 'analytics' | 'content' | 'posts' | 'settings';

/* ---------- Founder invitations (Pro) ---------- */

export type LeadStatus =
  | 'candidate' | 'replied' | 'invited' | 'responded' | 'setup_done' | 'first_result' | 'closed' | 'skipped';

/** A person the discovery agent investigated for a personal invitation. */
export interface Lead {
  ...
  createdAt: string;
  updatedAt: string;
  /** Joined from the linked public reply. */
  oppStatus: string | null;
  oppTitle: string | null;
  oppUrl: string | null;
  oppReplyCount: number | null;
}
...
export interface BrandDetail {
  ...
  projectPath: string;
  /** Listing facts for directory / launch-site forms (all optional). */
  pricingUrl: string;
  ...
  /** Founder invitations (Pro): who is worth a personal DM. */
...
export interface Settings {
  ...
  /** Weekly goals: local weekday the week starts on (0 = Sunday, 1 = Monday). */
  weekStartsOn: number;
  /** Assisted posting never clicks submit — pre-fill only, still marked posted. */
  dryRunPosts: boolean;
  /** Append utm_* params to own-site links when scheduling through Postiz. */
  utmTagging: boolean;
  cdpUrl: string;
...
export type GoalRule =
  | { type: 'content'; platform: string | null; source: string | null; titlePrefix: string | null; excludeTitlePrefix: string | null }
  | { type: 'post'; postType: string | null; destinationHost: string | null }
  | { type: 'reply'; platform: string | null }
  | { type: 'invitation'; platform: string | null };
...
/** One row an auto-rule matched this week; `excluded` = the user said "don't count this". */
export interface GoalAutoItem {
  kind: GoalRule['type'];
  id: string;
  title: string;
  at: string | null;
...
export interface GoalsWeek {
  ...
  endsAt: string;      // the instant the week rolls over (UTC ISO)
  startDate: string;   // local YYYY-MM-DD of the first day
  endDate: string;     // local YYYY-MM-DD of the last day
  weekStartsOn: number;
  label: string;
  labelLong: string;   // "Mon Sep 14 – Sun Sep 20"
  isCurrent: boolean;
  daysLeft: number;    // today included; 0 for other weeks
}

export interface GoalsBoard {
  week: GoalsWeek;
  goals: Goal[];
  totals: { goals: number; target: number; count: number; met: number };
}
...
export interface GoalPatch {
  ...
  target?: number;
  brandId?: string | null;
  rule?: Partial<GoalRule> & { type: GoalRule['type'] } | null;
  sortOrder?: number;
  archived?: boolean;
}
