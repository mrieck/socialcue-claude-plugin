...
import { ApiError, createGoal, deleteGoal, deleteGoalEntry, getGoals, logGoalEntry, patchGoal, seedGoals, setGoalItemExcluded } from '../api';
import type { Brand, Goal, GoalAutoItem, GoalPatch, GoalRule, GoalsBoard as Board } from '../types';
...
/** Local YYYY-MM-DD, for the once-a-minute "did the day change" check. */
 ...
 * see what Social Cue knows vs what they told it. Weeks start on the weekday
 * chosen in Settings (Sunday by default) in the bridge's local time; the
 * header says exactly when the current week rolls over. An auto-counted item
 * can be told not to count from the expanded card.
 */
export function GoalsBoard({ dataTick, brands }: Props) {
  ...
  // A tab left open across midnight must roll to the new week on its own —
  // the server picks the week at request time, so all it takes is a refetch
  // when the local date changes.
  useEffect(() => {
    let last = todayYmd();
    const t = window.setInterval(() => {
      const now = todayYmd();
      if (now !== last) { last = now; void refetch(); }
    }, 60_000);
  ...
  const setExcluded = (g: Goal, it: GoalAutoItem, excluded: boolean) =>
    run(`item:${it.id}`, async () => { replaceGoal(await setGoalItemExcluded(g.id, it.kind, it.id, excluded, weekDate)); });
  ...
  const weekEnds = week
    ? new Date(week.endsAt).toLocaleString(undefined, { weekday: 'long', month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' })
    : '';
      ...
      <div className="goals-head">
        ...
        {week && (
          <div className="week-nav">
            ...
            <span className="week-label" title={`Weeks start on ${['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'][week.weekStartsOn] ?? 'Sunday'} at midnight (change it in Settings → Discovery runs)`}>
              {week.labelLong}
              {week.isCurrent && (
                <span className="week-ends"> · rolls over {weekEnds} · {week.daysLeft} day{week.daysLeft === 1 ? '' : 's'} left</span>
              )}
            </span>
              ...
              >
                ...
                <div className="goal-count-row">
                  ...
                  <span className="goal-sub">
                    {g.autoCount} auto{g.excludedCount ? ` (${g.excludedCount} excluded)` : ''} · {g.manualCount} logged
                  </span>
                </div>
                ...
                <div className="goal-actions">
                  ...
                  <button type="button" className="copy" onClick={() => toggleOpen(g.id)}>
                    {isOpen ? 'Hide' : 'Details'}{g.autoItems.length + g.manualCount ? ` (${g.autoItems.length + g.manualCount})` : ''}
                  </button>
                </div>
                {isOpen && (
                  <EntryList
                    goal={g}
                    busy={busy}
                    onLog={note => logOne(g, note)}
                    onRemove={id => removeEntry(g, id)}
                    onExclude={(it, excluded) => setExcluded(g, it, excluded)}
                  />
                )}
              </div>
            );
...
function EntryList({ goal, busy, onLog, onRemove, onExclude }: {
  goal: Goal;
  busy: string | null;
  onLog: (note: string) => void;
  onRemove: (entryId: string) => void;
  onExclude: (item: GoalAutoItem, excluded: boolean) => void;
}) {
  ...
  return (
    <div className="goal-entries">
      {goal.rule && (
        <>
          <p className="muted small">Counted automatically ({goal.ruleLabel}) — click ✕ on anything that shouldn't count this week.</p>
          {!goal.autoItems.length && <p className="muted small">Nothing matched this week.</p>}
          {goal.autoItems.map(it => (
            <div key={`${it.kind}:${it.id}`} className={`goal-entry${it.excluded ? ' excluded' : ''}`}>
              <span className="goal-entry-at">{fmtEntryTime(it.at)}</span>
              <span className="goal-entry-note" title={it.title}>{it.title}</span>
              {it.status && it.status !== 'published' && <span className="badge">{it.status}</span>}
              {it.excluded ? (
                <button type="button" className="copy" title="Count it again" disabled={busy === `item:${it.id}`} onClick={() => onExclude(it, false)}>count</button>
              ) : (
                <button type="button" className="copy icon" title="Don't count this one" disabled={busy === `item:${it.id}`} onClick={() => onExclude(it, true)}>✕</button>
              )}
            </div>
          ))}
          <p className="muted small goal-entries-sep">Logged by hand</p>
        </>
      )}
      <form
...
function GoalModal({ goal, brands, onCancel, onSave, onArchive, onDelete }: {
  ...
  const submit = async (e: FormEvent) => {
    ...
    else if (ruleType === 'invitation') rule = { type: 'invitation', platform: platform || null };
    await guarded(() => onSave({ title: title.trim(), target: n, brandId: brandId || null, rule }));
  };

  const platformOptions = ruleType === 'reply' || ruleType === 'invitation' ? REPLY_PLATFORMS : CONTENT_PLATFORMS;
      ...
      <div className="goal-form-row">
        <label>
          ...
          <select value={ruleType} onChange={e => { setRuleType(e.target.value as RuleType); setPlatform(''); }}>
            ...
            <option value="invitation">Founder invitations marked invited (Pro)</option>
          </select>
        </label>
        {(ruleType === 'content' || ruleType === 'reply' || ruleType === 'invitation') && (
          <label>
            Platform
