...
const MarkdownModal = lazy(() => import('./MarkdownModal'));
import {
  ApiError,
  createContent,
  deleteContent,
...
interface EditorProps {
  ...
  onSaved: () => void;
  onClose: () => void;
  /** A brand-new item was persisted — bind the editor to it so later saves patch instead of re-creating. */
  onCreated: (item: ContentItem) => void;
}

function ContentEditor({ item, brands, channels, channelMap, onSaved, onClose, onCreated }: EditorProps) {
  ...
  /** Persist the draft without sending it anywhere (Save draft button, Done in the expanded editor). */
  ...
  return (
    <div className="detail content-editor">
      ...
      <section>
        <h3>
          Post{item?.format === 'markdown' && <span className="badge badge-source">markdown</span>}
          <button className="copy expand-btn" type="button" title="Open in a full-screen editor" onClick={() => setExpanded(true)}>
            Expand ⤢
          </button>
        </h3>
        {expanded && (
          <Suspense fallback={<div className="modal-backdrop"><div className="modal-card">Loading editor…</div></div>}>
            <MarkdownModal
              value={body}
              onChange={setBody}
              readOnly={sent}
              title={title}
              onClose={async () => { if (await saveDraft()) setExpanded(false); }}
            />
          </Suspense>
        )}
        {item?.notes && (
      ...
      {sent ? (
        ...
        <div className="post-row">
          <button className="copy" disabled={busy || !body.trim()} title="Save this draft without posting" onClick={() => void saveDraft()}>
            Save draft
          </button>
          <button
            className="post-btn"
        ...
        {items.length === 0 ? (
          ...
          <div className="opp-list">
            {items.map(item => (
              <div
                key={item.id}
                className={`opp-row content-row${editing !== 'new' && editing?.id === item.id ? ' selected' : ''}`}
                role="button"
                tabIndex={0}
              ...
              >
                ...
                <button className="copy" onClick={e => { e.stopPropagation(); remove(item); }}>Delete</button>
              </div>
            ))}
          </div>
        )}
