...
export const POSTIZ_REFERRAL_URL = 'https://postiz.com';
...
function ContentEditor({ item, brands, channels, channelMap, onSaved, onClose }: EditorProps) {
  ...
  const buildPatch = (): ContentPatch => {
    // Postiz per-channel settings; Reddit needs __type + subreddit + title.
    const settings: Record<string, unknown> = {};
    for (const id of selected) {
      const ch = channels?.find(c => c.id === id);
      if (ch?.identifier?.includes('reddit') && subreddits[id]) {
        settings[id] = {
          __type: 'reddit',
