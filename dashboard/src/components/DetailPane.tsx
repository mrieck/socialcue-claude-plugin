  ...
  const postInBrowser = async (submit = false) => {
    ...
    try {
      ...
      if (err instanceof ApiError && err.status === 403) {
        // Only fires when the UI's pro flag is stale vs the bridge's gate.
        setPostMsg({ text: 'Assisted posting is a Pro feature.', ok: false });
        onOpenLicense();
    ...
    return (
      <aside className="detail">
        ...
        <section>
          ...
          {/* A mis-clicked ✓ is common; undo puts the row back in the queue as
              reviewed and clears the posted date so weekly counts stay honest. */}
          <div className="post-row">
            <button type="button" className="copy" onClick={() => onSetStatus(o.id, 'reviewed')}>
              Undo — not actually posted
            </button>
          </div>
        </section>
      </aside>
    ...
    <aside className="detail">
      ...
      <div className="post-row">
        <button
          className="post-btn"
          disabled={posting}
          onClick={() => (pro ? void postInBrowser(o.kind !== 'recommendation') : onOpenLicense())}
          title={pro
