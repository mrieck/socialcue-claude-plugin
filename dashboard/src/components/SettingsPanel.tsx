...
const PLATFORM_CHOICES = ['reddit', 'hackernews', 'x', 'linkedin', 'producthunt', 'indiehackers', 'threads'];
...
const MIN_SCORE_HINTS: Record<number, string> = {
  4: 'wide net — expect some threads that only loosely fit',
  5: 'loose — more threads, more skimming',
  6: 'default — solid fits only',
  7: 'tight — strong fits, fewer per run',
  8: 'very tight — only the obvious wins',
};
...
function RunSetupSection({ settings }: { settings: Settings }) {
  const initial: RunForm = {
    ...
    maxLeadsPerRun: settings.maxLeadsPerRun ?? 3,
    weekStartsOn: settings.weekStartsOn ?? 0,
    dryRunPosts: settings.dryRunPosts ?? false,
    utmTagging: settings.utmTagging ?? true,
  };
  ...
  const save = async () => {
    ...
    try {
      await saveRunSettings({
        ...
        maxLeadsPerRun: form.maxLeadsPerRun,
        weekStartsOn: form.weekStartsOn,
        dryRunPosts: form.dryRunPosts,
        utmTagging: form.utmTagging,
      });
      setSnapshot(form);
  ...
  return (
    <section className="settings-block">
      ...
      <div className="run-field">
        <label>
          Minimum score <span className="hint">lower it when runs come back thin; the queue still sorts by score</span>
        </label>
        <div className="slider-row no-label">
          <input
            type="range"
          ...
          />
          <span className="slider-value">{form.minScore}/10 · {MIN_SCORE_HINTS[form.minScore] ?? ''}</span>
        </div>
      </div>

      <div className="run-field">
        <label>
          Leads per run <span className="hint">Pro · people investigated for a personal invitation when their post explicitly asks for what a brand does; 0 turns it off</span>
        </label>
        <div className="slider-row no-label">
          <input
            type="range"
            ...
            onChange={e => setForm(f => ({ ...f, maxLeadsPerRun: Number(e.target.value) }))}
          />
          <span className="slider-value">{form.maxLeadsPerRun === 0 ? 'off' : `up to ${form.maxLeadsPerRun} per run`}</span>
        </div>
      </div>

      <div className="run-field">
        <label>
          Goals week starts on <span className="hint">the Analytics board buckets the week from midnight on this day; the header shows exactly when it rolls over</span>
        </label>
        <div className="slider-row no-label">
          <select value={form.weekStartsOn} onChange={e => setForm(f => ({ ...f, weekStartsOn: Number(e.target.value) }))}>
            <option value={0}>Sunday (Sun → Sat)</option>
            <option value={1}>Monday (Mon → Sun)</option>
          </select>
        </div>
      </div>
      ...
      <div className="run-field">
        <label>
          Auto-tag links (UTM){' '}
          <span className="hint">
            When a post is scheduled through Postiz, links to your own brand sites get
            utm_source (the channel), utm_medium=social, utm_campaign (the brand) and
            utm_content (this post), so your site analytics can tell which post brought
            a visitor. Links to other sites, and links you tagged yourself, are left alone.
          </span>
        </label>
        <button
          type="button"
          role="switch"
          aria-checked={form.utmTagging}
          className={`switch${form.utmTagging ? ' on' : ''}`}
          onClick={() => setForm(f => ({ ...f, utmTagging: !f.utmTagging }))}
        >
          <span className="knob" />
        </button>
      </div>
...
function BrandEditForm({ b, onCancel, onSaved }: {
  ...
  const [f, setF] = useState({
    ...
    pricingUrl: b.pricingUrl ?? '',
    pricingSummary: b.pricingSummary ?? '',
    repoUrl: b.repoUrl ?? '',
    launchedOn: b.launchedOn ?? '',
    founders: b.founders ?? '',
    socialLinks: Object.entries(b.socialLinks ?? {}).map(([k, v]) => `${k}=${v}`).join('\n'),
    idealCustomer: b.idealCustomer ?? '',
    requirements: b.requirements ?? '',
    disqualifiers: b.disqualifiers ?? '',
  });
  const [busy, setBusy] = useState(false);
  ...
  const save = async () => {
    ...
    try {
      const saved = await patchBrand(b.id, {
        ...
        pricingUrl: f.pricingUrl.trim(),
        pricingSummary: f.pricingSummary.trim(),
        repoUrl: f.repoUrl.trim(),
        launchedOn: f.launchedOn.trim(),
        founders: f.founders.trim(),
        idealCustomer: f.idealCustomer.trim(),
        requirements: f.requirements.trim(),
        disqualifiers: f.disqualifiers.trim(),
        socialLinks: Object.fromEntries(
          f.socialLinks.split('\n').map(l => l.trim()).filter(Boolean)
            .map(l => { const i = l.indexOf('='); return i > 0 ? [l.slice(0, i).trim().toLowerCase(), l.slice(i + 1).trim()] : null; })
  ...
  return (
    <form className="brand-edit" onSubmit={e => { e.preventDefault(); void save(); }}>
      ...
      <div>
        <label>Listing facts <span className="muted">(optional — directory forms are filled from these instead of guesses; blank = left empty)</span></label>
        <input type="text" value={f.pricingUrl} placeholder="Pricing page URL" onChange={set('pricingUrl')} />
        <input type="text" value={f.pricingSummary} placeholder="Pricing summary, e.g. Free plugin; Pro $19/mo" onChange={set('pricingSummary')} />
        <input type="text" value={f.repoUrl} placeholder="Public repo URL" onChange={set('repoUrl')} />
        <input type="text" value={f.launchedOn} placeholder="Launched on (YYYY-MM-DD)" onChange={set('launchedOn')} />
        <input type="text" value={f.founders} placeholder="Founder(s), comma-separated" onChange={set('founders')} />
        <textarea rows={2} value={f.socialLinks} placeholder={'Other links, one per line: linkedin=https://…\nx=https://…'} onChange={set('socialLinks')} />
      </div>
      <div>
        <label>Ideal customer <span className="muted">(Pro invitations — who is worth a personal DM; blank = inferred conservatively)</span></label>
        <textarea rows={2} value={f.idealCustomer} placeholder="e.g. solo founder with a released SaaS who does their own marketing" onChange={set('idealCustomer')} />
        <input type="text" value={f.requirements} placeholder="Requirements, stated up front in every DM — e.g. needs a Claude Code subscription" onChange={set('requirements')} />
        <input type="text" value={f.disqualifiers} placeholder="Disqualifiers — e.g. agencies, students, no released product" onChange={set('disqualifiers')} />
      </div>
