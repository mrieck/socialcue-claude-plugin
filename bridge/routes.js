...
import { handleGoalsApi } from './goals-routes.js';
import { handleLeadApi } from './lead-routes.js';
import { goalsShortCount } from '../lib/goals.js';
...
export async function handleApi(req, res, ctx) {
  ...
  if (p === '/api/changes' && method === 'GET') {
    json(res, 200, {
      ...
      // `analytics` = active weekly goals still short this week (Analytics
      // badge); `submitted` stays for a stale dashboard build.
      counts: { ...db.opportunityCounts(), analytics: goalsShortCount() },
    });
    return true;
  }
  ...
  if (p === '/api/settings' && method === 'GET') {
    ...
    json(res, 200, {
      ...
      minScore: c.minScore ?? 6,
      maxLeadsPerRun: c.maxLeadsPerRun ?? 3,
      weekStartsOn: c.weekStartsOn ?? 0,
      dryRunPosts: c.dryRunPosts === true,
      utmTagging: c.utmTagging !== false,
  ...
  if (p === '/api/settings/run' && method === 'PATCH') {
    ...
    if (body?.minScore !== undefined) {
      ...
      if (!Number.isInteger(n) || n < 1 || n > 10) errors.push('minScore must be an integer between 1 and 10');
      else patch.minScore = n;
    }
    if (body?.maxLeadsPerRun !== undefined) {
      const n = Number(body.maxLeadsPerRun);
      if (!Number.isInteger(n) || n < 0 || n > 10) errors.push('maxLeadsPerRun must be an integer between 0 and 10');
      else patch.maxLeadsPerRun = n;
    }
    if (body?.weekStartsOn !== undefined) {
      const n = Number(body.weekStartsOn);
      if (!Number.isInteger(n) || n < 0 || n > 6) errors.push('weekStartsOn must be an integer between 0 (Sunday) and 6 (Saturday)');
      else patch.weekStartsOn = n;
    }
    ...
    if (body?.utmTagging !== undefined) {
      if (typeof body.utmTagging !== 'boolean') errors.push('utmTagging must be a boolean');
      else patch.utmTagging = body.utmTagging;
    }
    if (errors.length) {
    ...
    if (!Object.keys(patch).length) {
      json(res, 400, { error: 'nothing to update — pass platforms, strategyWeights, engagementRatio, maxTurnsPerPlatform, minScore, maxLeadsPerRun, weekStartsOn, dryRunPosts and/or utmTagging' });
      return true;
    }
    let saved;
    try {
    ...
    json(res, 200, {
      ...
      maxLeadsPerRun: saved.maxLeadsPerRun,
      weekStartsOn: saved.weekStartsOn ?? 0,
      dryRunPosts: saved.dryRunPosts === true,
      utmTagging: saved.utmTagging !== false,
    });
    return true;
  }
  ...
  // Weekly goals (Analytics tab) — bridge/goals-routes.js.
  if (await handleGoalsApi(req, res, { json, readBody })) return true;
  if (await handleLeadApi(req, res, { json, readBody })) return true;
