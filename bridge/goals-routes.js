 ...
 *   PATCH  /api/goals/:id/items/:kind/:itemId      { excluded: bool }  don't count / count again an auto item
 */
import * as db from '../lib/db.js';
import * as cfg from '../lib/config.js';
import { bumpWrites } from './changes.js';
import { RULE_TYPES, configuredWeekStart, goalSummary, seedStarterGoals, validateRule, weekBounds, weekSummary, parseWeekParam } from '../lib/goals.js';
...
function currentGoal(id, weekParam) {
  ...
  if (!row) return null;
  const week = weekBounds(parseWeekParam(weekParam) ?? new Date(), configuredWeekStart());
  let brands = [];
...
export async function handleGoalsApi(req, res, { json, readBody }) {
  ...
  const itemPatch = p.match(/^\/api\/goals\/([^/]+)\/items\/([^/]+)\/([^/]+)$/);
  if (itemPatch && method === 'PATCH') {
    const id = decodeURIComponent(itemPatch[1]);
    const kind = decodeURIComponent(itemPatch[2]);
    const itemId = decodeURIComponent(itemPatch[3]);
    if (!db.getGoalById(id)) { json(res, 404, { error: `no goal ${id}` }); return true; }
    if (!RULE_TYPES.includes(kind)) { json(res, 400, { error: `kind must be one of ${RULE_TYPES.join('|')}` }); return true; }
    let body;
    try { body = await parseBody(); } catch (err) { badBody(err); return true; }
    if (typeof body?.excluded !== 'boolean') { json(res, 400, { error: 'excluded must be a boolean' }); return true; }
    try {
      if (db.setGoalExclusion(id, kind, itemId, body.excluded)) bumpWrites();
