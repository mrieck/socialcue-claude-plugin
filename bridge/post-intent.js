...
/**
 * Open `url` in the dedicated Chrome (launching it if needed) and return the
 * connected browser + the page, brought to front. The caller must
 * `browser.close()` (disconnect — the user's window stays open). Shared by
 * assisted posting and the Invitations tab's "open profile" action.
 * Throws on connection-level failures only.
 */
export async function openInDedicatedBrowser(url, config) {
  ...
  try {
    ...
    if (launched) {
      ...
      page = context.pages().find(pg => norm(pg.url()) === norm(url)) ?? null;
...
/** Open a URL in the dedicated Chrome and disconnect. Same serialization as post intents. */
export function openUrlIntent(url, config) {
  const run = async () => {
    log(`open start url=${url}`);
    const { browser } = await openInDedicatedBrowser(url, config);
    await browser.close().catch(() => {});
    return { opened: true, message: 'Opened in the Social Cue browser.' };
  };
  const p = queue.then(run, run);
  queue = p.catch(() => {});
  return p;
...
async function doPostIntent({ platformUrl, suggestedReply }, config, { submit = false } = {}) {
  ...
  const { browser, page } = await openInDedicatedBrowser(platformUrl, config);
...
async function prefillReply(page, reply) {
  ...
  if (host === 'news.ycombinator.com') {
    ...
    // Markdown textarea: a lone "\n" renders as a space, so every line break
    // in the draft becomes a blank line.
    return fillTextarea(page, page.locator('textarea[name="text"]').first(), markdownParagraphs(reply));
  }

  if (host === 'old.reddit.com') {
    return fillTextarea(page, page.locator('.commentarea textarea[name="text"]').first(), markdownParagraphs(reply));
  }
  ...
  if (host === 'reddit.com' || host.endsWith('.reddit.com')) {
    ...
    // Lexical swallows "\n" inside one insertText and posts a single block, so
    // the fill types line by line with a real Enter between lines.
    ...
    return fillContentEditable(page, box, reply, 'paragraph');
  }
  ...
  if (host === 'twitter.com' || host === 'x.com') {
    ...
    return fillContentEditable(page, box, reply, 'literal');
  }

  if (host === 'threads.com' || host === 'threads.net') {
    // Post permalink pages render an inline Lexical reply composer
    // (div[contenteditable][role=textbox], aria-placeholder "Reply to <user>...")
    // once the SPA hydrates — wait for it like the X path. Submit is the
    // generic branch: a "Post" button appears once text registers.
  ...
  if (host === 'indiehackers.com') {
    ...
    if (await box.isVisible().catch(() => false)) return fillContentEditable(page, box, reply, 'paragraph');
    return fillTextarea(page, page.locator('textarea').first(), markdownParagraphs(reply));
  }
...
/**
 * mode: 'paragraph' — runs of newlines become one Enter (rich-text editors
 * where each Enter is its own paragraph); 'literal' — one Enter per "\n"
 * (plain-text composers where a blank line is a real blank line).
 */
async function fillContentEditable(page, box, reply, mode = 'paragraph') {
  await box.click({ timeout: 3000 });
  try {
    await insertLines(page, reply, mode);
    return 'insertText';
  } catch {
