...
If the brief includes a **"Performance check-ins"** list (things the user
posted that are due a check), do these FIRST — they're quick and the browser is
already up. Each row has a `kind`:
- `reply` — the user replied in someone else's thread. `navigate` to the URL,
  `read_page`, find the **user's own comment** (the author matching the
  logged-in account — usually highlighted or showing edit/delete controls),
  read its current score and count its direct replies. Reddit shows points on
  every comment; HN shows a score only on the user's own comments (you're in
  their logged-in browser, so it's visible).
- `product_post` — the URL **is the user's own post** (a meme on a subreddit, a
  directory listing, a Show HN). Read the post's score/upvotes and its comment
  count; note whether it's still up, removed by mods, or flaired.
If the thread, comment or post is gone, record a check with `note: "removed"`
and move on. Then call `record_performance(oppId, upvotes, replyCount, note)`
with the row's id — omit numbers you can't see rather than guessing.
...
## Founder invitations (Pro, when the brief includes them)
If the brief has a **"FOUNDER INVITATIONS"** section, some threads are also
candidates for a personal invitation: the author is asking, in their own words,
for what one brand does. The section states the cap per run and each brand's
ideal customer; the scoring-brain skill's "Founder invitations" section has the
rubric and DM rules. Per person, in this order:
1. `collect_opportunity` the public reply for the post as usual (the invitation
   only ever goes out after that reply is posted).
2. `open_tab` their profile URL → `read_page` (refs reset on every tab change) →
   skim the bio and recent posts; if they link a product, `navigate` to it once.
   **At most 4 turns.** Then `close_tab` and `switch_tab` back to where you were.
3. `collect_lead` with the post URL, handle, profileUrl, the one brandId, their
   quoted ask, an honest fitScore + fitSummary, 2–6 short evidence facts, any
   disqualifiers, and a DM draft (≤ 80 words; quotes their post; names their
   product; states the brand's requirement up front; one yes/no ask; offers setup
   help and asks for blunt feedback; promises nothing).
Skip anyone in the section's "already investigated" list, stop at the cap, and
never follow, like, or message anyone during discovery. If `collect_lead` says
`pro_required`, drop invitations for the rest of the run.
